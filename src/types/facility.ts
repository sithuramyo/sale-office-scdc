...
export type HotspotKind='cctv'|'wifi'|'network'|'sensor'|'access'|'fire'|'rack'|'iot'|'meeting';
export interface Hotspot {id:string;type:HotspotKind;label:string;position:[number,number,number];buildingId?:string;floorId?:string;status:'online'|'warning'|'offline'|'unknown'}
...
export interface ViewerState {selectedEntity:string|null;hoveredEntity:string|null;selectedBuilding:string|null;selectedFloor:string|null;activeLayers:Record<string,boolean>;activeCameraPreset:string;viewerMode:ViewerMode;drawerOpen:boolean;dayNightMode:'day'|'night';isolated:boolean}
